    ...
    virtual task body ();
        ...
        for (int i = 0; i<300; i++) begin
            ...
            `uvm_info("SEQ", $sformatf(
                ...
                spi_item.write, spi_item.addr_reg, spi_item.num_bytes), UVM_NONE)
...
class spi_driver extends uvm_driver #(spi_seq_item);
    ...
    task  send_byte(byte b);
        ...
        `uvm_info("DRV", $sformatf(
            "Drive DUT write:%0d, addr_reg:%0d, num_bytes:%0d",
            spi_item.write, spi_item.addr_reg, spi_item.num_bytes), UVM_NONE)
        @(posedge s_if.clk);
        @(posedge s_if.clk);
        @(posedge s_if.clk);
        ...
        forever begin
            ...
            `uvm_info("MON", $sformatf(
                "sampled write:%0d, addr_reg:%0d, num_bytes:%0d, tx_byte : %d, rx_byte = %d",
                 spi_item.write, spi_item.addr_reg, spi_item.num_bytes, spi_item.tx_bytes , spi_item.rx_bytes), UVM_NONE)
...
class spi_scoreboard extends uvm_scoreboard;
    ...
    virtual function void write(spi_seq_item item);
        ...
        `uvm_info("SCB", $sformatf("sampled write:%0d, addr_reg:%0d, num_bytes:%0d, tx_byte : %d, rx_byte = %d",
         spi_item.write, spi_item.addr_reg, spi_item.num_bytes, spi_item.tx_bytes , spi_item.rx_bytes), UVM_NONE)

        if(addr_affter == 1 && WR == 1) begin
            golden_reg[slv_addr] = spi_item.tx_bytes;
             `uvm_info("SCB", $sformatf("Write success addr : %d, data : %d", slv_addr, spi_item.tx_bytes ), UVM_LOW)
             slv_addr ++;
             count_number --;
        end
        ...
        if(addr_affter == 1 && WR == 0) begin
            ...
            `uvm_info("SCB", $sformatf("*** TEST PASS *** , rx_Data : %0d, data : %0d, addr : %0d",
             spi_item.rx_bytes, golden_reg[slv_addr], slv_addr), UVM_NONE);
            slv_addr ++;
            count_number --;
            end
            else begin
                `uvm_error("SCB", $sformatf("*** TEST FAIL *** , rx_Data : %0d, data : %0d, addr : %0d",
                 spi_item.rx_bytes, golden_reg[slv_addr], slv_addr))
                slv_addr ++;
                count_number --;
            end
        end
        ...
        if(addr_affter == 0) begin
            ...
            `uvm_info("SCB", $sformatf("*** data_start *** , rx_Data : %0d, data : %0d, addr : %0d",
             spi_item.rx_bytes, golden_reg[slv_addr], slv_addr), UVM_NONE);
        end
...
class spi_envirnment extends uvm_env;
    ...
    virtual function void build_phase(uvm_phase phase);
        super.build_phase(phase);
        spi_sco = spi_scoreboard::type_id::create("SCB", this);
